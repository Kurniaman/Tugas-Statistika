...
emerging health trends. For example, respondents are asked about their diet and weekly physical activity, their HIV/AIDS status, possible tobacco use, and even their level of healthcare coverage. The BRFSS Web site 
([http://www.cdc.gov/brfss](http://www.cdc.gov/brfss)) contains a complete description of the survey, including the research questions that motivate the study and many interesting results derived from the data.
...
```{r summary of height & interquartile}
summary(cdc$height)
70-64
```
```{r summary of age & interquartile}
summary(cdc$age)
57-31
```
```{r relative frequency distribution gender}
table(cdc$gender)/2000
```
```{r relative frequency distribution exerany}
table(cdc$exerany)/2000
```
```{r count of males in the sampe}
table(cdc$gender)
```
conlusion : dari data exerany didapatkan bahwa, yang melakukan exercise lebih banyak dari pada yang 
tidak melakukan exercise
...
smoke_gender <- table(cdc$smoke100, cdc$gender)
mosaicplot(smoke_gender)
```
...
Answer:
The mosaic plot reveal that on data which people didnt smoking, total of female who didnt smoke 
more than male. Otherwise, in data which people smoking between 100 smoke or more, total of male
who smoking is more than total sample of female.
...
cdc[1:10,]
#head(cdc,10) --> untuk menampilkan head 10 teratas
```
...
males_andover30 <- subset(cdc, cdc$gender == "m" & cdc$age>30)
head(males_andover30)
```
...
males_orover30 <- subset(cdc, cdc$gender == "m" | cdc$age>30)
head(males_orover30)
```
...
5. Quar. 1 : -21.00
   Quar. 2 (Median) : -10.00    
   Quar. 3 : 0.00
b) Dengan plot, kita bisa mendapatkan insight yakni kebanyakan berat badan orang saat ini sudah mendekati berat badan yang mereka inginkan. Meskipun begitu, ada beberapa orang yang rentang antara berat badan yang diinginkan berbeda jauh dengan berat badan mereka saat ini (outlier). 
...
pria <- cdc$gender =="m"
wanita <- cdc$gender =="f"
memandang_berat <- wdiff <0 |wdiff>0
summary(pria&memandang_berat)
summary(wanita&memandang_berat)
```
```{r}
...
Answer : 
Dari hasil numerical summaries, didapat bahwa perempuan lebih banyak dalam memperhatikan berat badan dengan jumlah 7516 dibanding pria yang sebanyak 5248.
